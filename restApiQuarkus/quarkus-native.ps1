...
$csv_path = "metrics-output\docker_native_metrics_$(Get-Date -Format 'yyyyMMddHHmmss').csv"

"Time,Container Name,CPU %,Memory Usage" | Out-File -FilePath $csv_path -Encoding UTF8

while ($true) {
    $timestamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
    $dockerStats = docker stats api-native-benchmark --no-stream --format "{{.Name}},{{.CPUPerc}},{{.MemUsage}}"
